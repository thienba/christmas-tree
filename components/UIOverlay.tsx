...
export const UIOverlay: React.FC<UIOverlayProps> = ({ mode, onToggle, onPhotosUpload, hasPhotos, uploadedPhotos, isSharedView }) => {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      setUploadProgress('Getting upload addresses...');
      ...
      if (urlsResponse.status === 404) {
        ...
        if (isLocalDev) {
          ...
          try {
            ...
            setShareError('Image data too large, please reduce the number of photos or size');
            return;
          }
        } else {
          throw new Error('API not configured, please check deployment settings');
        }
      }
      ...
      if (!urlsResponse.ok) {
        throw new Error(urlsData.error || 'Failed to get upload addresses');
      }
      ...
      if (!completeResponse.ok) {
        throw new Error(completeData.error || 'Failed to save share information');
      }
  ...
  return (
    <div className="absolute top-0 left-0 w-full h-full pointer-events-none z-10">
      ...
      <div className="absolute bottom-8 right-8 flex flex-col items-end gap-4 pointer-events-auto">
        ...
        {/* Shared View: Show "Make My Christmas Tree" button */}
        {isSharedView && (
          <button
